...
// Tipo para as props que o formulário irá receber (alinhado aos outros forms)
type FormProps = {
    loggedUser?: {
        login: string;
...
const TranspixForm: React.FC<FormProps> = ({ loggedUser, readOnlyName = false }) => {
    ...
    // Texto principal
        const fretesTexto = [
            ...
            ...extraFretes.map(
                f => `- Número da Cotação: ${f.quotationNumber}\n- Número da NF: ${f.nfNumber}\n- Valor: R$ ${f.value}`
            )
        ].join('\n\n');
